...
class LogicaJuego(QObject):
    ...
    def configurar_timers(self) -> None:
        #Primer Timer
    ...
    def mover_plataforma(self, tecla: str) -> None:
        if tecla in p.TECLA_IZQUIERDA or tecla in p.TECLA_DERECHA:
            nueva_posicion = self.plataforma.mover(tecla)
            self.senal_mover_plataforma.emit(nueva_posicion)
        elif tecla in p.TECLA_CHEATCODE_KO:
            self.cheatcode()
