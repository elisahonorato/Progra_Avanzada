...
class ProtagonistaDisfrazado: 
    ...
    def recuperar_casas_BFS_iterativo(self, casa_inicial: Casa)-> list:
        ...
        #COMPLETAR
        visitados = set()
        queue = deque([(casa_inicial, 0)])
        lista_dic = []
        while len(queue) > 0:
            vertice, largo = queue.popleft()
            if vertice in visitados:
                continue
            visitados.add(vertice)
            dict_actual = {"casa": vertice, "distancia": largo}
            if vertice.hijo_derecho is not None:
                queue.append((vertice.hijo_derecho, largo + 1))
            if vertice.hijo_izquierdo is not None:
                queue.append((vertice.hijo_izquierdo, largo + 1))
            lista_dic.append(dict_actual)
        return lista_dic
    ...
    def filtrar_casas(self, listado_casas:list)->list:
        ...
        for dic_casa in listado_casas:
            print(dic_casa["casa"].decoracion)
